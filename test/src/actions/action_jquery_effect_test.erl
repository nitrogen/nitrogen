...
basic_test_() ->
  ...
  ?_assertEqual("wf_current_path=''; $(obj('_target_path')).effect('none', { Key1: 'Value1',Key2: 'Value2' }, \"a_speed\");",
                new_jquery_effect_1('effect')),
  ?_assertEqual("wf_current_path=''; $(obj('_target_path')).toggle('none', { Key1: 'Value1',Key2: 'Value2' }, \"a_speed\");",
                new_jquery_effect_1('toggle')),
  ?_assertEqual("wf_current_path=''; $(obj('_target_path')).addClass('a_class', \"a_speed\");",
                new_jquery_effect_1('add_class')),
  ?_assertEqual("wf_current_path=''; $(obj('_target_path')).removeClass('a_class', \"a_speed\");",
                new_jquery_effect_1('remove_class')),
  ?_assertEqual("wf_current_path=''; $(obj('_target_path')).animate({ Key1: 'Value1',Key2: 'Value2' }, \"a_speed\", 'some_easing');", new_jquery_effect_1('animate'))
  ].

to_js_test_() ->
  [
   ?_assertEqual("{ alist: 'backgroundblue' }", action_jquery_effect:to_js([{alist,["background","blue"]}])),
   ?_assertEqual("{ akey: 'WHAT IS THIS SUPPOSED BE?' }", action_jquery_effect:to_js([{akey,"WHAT IS THIS SUPPOSED BE?"}])),
   ?_assertEqual("{ akey: true }", action_jquery_effect:to_js([{akey,true}])),
   ?_assertEqual("{ akey: false }", action_jquery_effect:to_js([{akey,false}])),
